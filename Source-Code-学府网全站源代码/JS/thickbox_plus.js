...
function TB_show(caption, url) { //function called when the user clicks on a thickbox link
 	try {
		...
		$("body").append("<div id='TB_load'><div id='TB_loadContent'><iframe=" + url + " scrolling='no' frameborder='0' width='' height=''></div></div>");
		 
		$("#TB_overlay").show();
		var urlString = /.jpg|.jpeg|.png|.gif|.html|.htm|.aspx/g;
		...
		if(urlType == '.htm' || urlType == '.html'||urlType=='.aspx'){//code to show html pages
			...
			$("#TB_closeWindowButton").click(TB_remove);
			$("#TB_ajaxContent").append("<iframe src='" + url + "' scrolling='no' frameborder='0' style='width:" + ajaxContentW + "px;height:" + ajaxContentH + "px;'>");
			TB_position();
			$("#TB_load").remove();
			$("#TB_window").slideDown("normal");
...
$("#TB_overlay").css("height", yScroll + "px"); //yScroll,y
}
