<%@ Page language="c#" %>
<script runat="server" charset="gb2312">
...
private void Page_Load(object sender, System.EventArgs e) 
{
	...
	if (isframe != "") {
		...
		string rif = "" + Request.QueryString["rif"];
		string cif = "" + Request.QueryString["cif"];	
        
        //另外添加的添加文件夹的代码**************************************************************
        string path=@"NewsImages\";
        try
        {
            if (Session["rank"].ToString() != "3")
            {
                path = path + "$" + Session["username"].ToString() + "\\";
            }
            else
            {
                path = path + Session["username"].ToString() + "\\";
        ...
        {
            System.IO.DirectoryInfo di = System.IO.Directory.CreateDirectory(System.AppDomain.CurrentDomain.BaseDirectory.ToString() + path);
        }
        //****************************************************************************************
		if (cif != "" && rif != "") {
            RootImagesFolder.Value = path;
            CurrentImagesFolder.Value =path;
		} else {
...
public void DisplayImages() {
    ...
    {
		gallerymessage.Text = NoImagesMessage;
    ...
    else 
    {
		...
		foreach (string ImageFile in FilesArray) {
			...
			try {
                ...
                //Response.Write(ImageFileLocation.Replace("\\", "/") + "','" + myImage.Width.ToString() + "','" + myImage.Height.ToString() + "');");
				// landscape image
