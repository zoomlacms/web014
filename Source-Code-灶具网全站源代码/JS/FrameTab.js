...
function SwitchIframe(iFrameTab) {
    ...
    if (newMainFrame.length <= 0) {//是否新建标签
        newMainFrame = jQuery("#main_right_frame").prepend(jQuery("#iframeMainTemplate").html())
				.find("[tabid=0]").attr({ "tabid": tabId, "src": "Page.aspx", "id": "main_right", "name": "main_right" })
				.css("display", "block");
    } else {
...
function InitNewFrameTab() {
    jQuery("#newFrameTab").click(function() {
        ...
        jQuery('<li id="iFrameTab' + (++lastFrameTabId) + '" ><a href="javascript:"><span id="frameTabTitle">新建选项卡</span><a class="closeTab"><img border="0" src="/images/tab-close.gif"/></a></a></li>')
...
function SetTabTitle(tarFrame){ 
    ...
    var subTitle = title = title || "新建选项卡";
