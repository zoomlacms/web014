...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn.gif)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn_over.gif)\'" style="margin: 7px 0 0; position: relative; cursor: pointer; float: right; height: 17px; width: 28px; background-image: url(' + IMAGESPATH + 'dialog_closebtn.gif);' + (this.ShowCloseButton ? "" : "display:none;") + '"  title=\'关闭操作\' alt=\'关闭操作\'></div>\
      <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);gotourl(\'' + this.displacePath() + '\');" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_returnbtn.gif)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_returnbtn_over.gif)\'" style="margin: 7px 0 0; position: relative; cursor: pointer; float: right; height: 17px; width: 28px; background-image: url(' + IMAGESPATH + 'dialog_returnbtn.gif);' + (this.ShowCloseButton ? "" : "display:none;") + '"  title=\'返回全屏\' alt=\'返回全屏\'></div>\
      <div onclick="window.parent.frames[\'_DialogFrame_0\'].location.reload();" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_refresh.gif)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_refresh_over.gif)\'" style="margin: 7px 0 0; position: relative; cursor: pointer; float: right; height: 17px; width: 28px; background-image: url(' + IMAGESPATH + 'dialog_refresh.gif);' + (this.ShowCloseButton ? "" : "display:none;") + '"  title=\'刷新页面\' alt=\'刷新页面\'></div>\
        </td>\
    ...
    if (this.URL) {
        ...
        innerFrameOnload = function () {
            try {
                ...
                //alert("可能存在访问限制，不能获取到iframe中的对象。");
            }
